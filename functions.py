...
class Bots:
    ...
    def RSI_bot(self, overbought, oversold, window):
        # Creates a copy of the DataFrame to avoid modifying the original.
        trade_signals = self.ohlcv_df.copy()
        ...
        for index, row in trade_signals.iterrows():
            # Evaluates literals.
            rsi_above_overbought_threshold = trade_signals.at[index, 'rsi'] > overbought
            rsi_below_oversold_threshold = oversold > trade_signals.at[index, 'rsi']
    ...
    def VWAP_bot(self, window):
        ...
        # The VWAP values are computed from the high, low, close and volume prices.
